# Somnigraph Tracker

A public idea tracker for the Somnigraph map editor for Build/Duke Engine and Contour. This repository uses GitHub Issues to manage and track ideas, feature requests, and project planning.

## Purpose

This repository serves as a centralized location for:
- 💡 Collecting and discussing project ideas
- 🚀 Tracking feature requests
- 🐛 Reporting and tracking bugs
- 💬 Community discussions and feedback

## How to Use

### Submit an Idea

1. Navigate to the [Issues](../../issues) tab
2. Click "New Issue" 
3. Choose an appropriate template (Idea, Feature Request, Bug Report, etc.)
4. Fill out the template with as much detail as possible
5. Submit the issue for discussion

### Participate in Discussions

- Browse existing issues to see what's being discussed
- Add your thoughts, suggestions, or concerns to existing issues
- Vote on issues using 👍 reactions to show support
- Follow issues to get notified of updates

## Community Guidelines

We welcome contributions from everyone! Please:
- Be respectful and constructive in discussions
- Search for existing issues before creating a new one
- Provide clear and detailed information in your submissions
- Stay on topic and keep discussions focused

## Getting Started

👉 [View all ideas and requests](../../issues)

👉 [Submit a new idea](../../issues/new)

---

**Note:** This is an idea tracker repository.
